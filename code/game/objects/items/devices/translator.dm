...
/obj/item/device/universal_translator
	...
	var/universal = 0 //YW Edit. Can it understand ANY language?
...
/obj/item/device/universal_translator/attack_self(mob/user)
	if(!listening) //Turning ON
		...
		if(langset)
			if(langset && ((langset.flags & NONVERBAL) || (langset.flags & HIVEMIND) || (!langset.machine_understands && !universal))) //YW Edit
...
/obj/item/device/universal_translator/hear_talk(var/mob/speaker, var/message, var/vrb, var/datum/language/language)
	...
	if (!language.machine_understands && !universal) //YW Edit
