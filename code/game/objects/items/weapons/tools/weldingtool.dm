...
/obj/item/weldingtool/afterattack(obj/O as obj, mob/user as mob, proximity)
	...
	if (istype(O, /obj/structure/reagent_dispensers/fueltank) && get_dist(src,O) <= 1)
		if(!welding && max_fuel)
			...
			//Yawn edit: removed weldertank booms you are gaypwnd
			to_chat(user, span_danger("You begin welding on the fueltank and with a moment of lucidity you realize, you are gaypwnd."))
			//End yawn edit
			return
	if (src.welding)
		remove_fuel(1)
