...
/datum/event/rogue_drone/announce()
	var/msg
	var/rng = rand(1,5)
	//YW Edit Start TFF 14/1/20 - Virgo 3b -> Borealis 2
	switch(rng)
		if(1)
			msg = "A combat drone wing operating in close orbit above Borealis 2 has failed to return from a anti-piracy sweep.  If any are sighted, \
			approach with caution."
		if(2)
			msg = "Contact has been lost with a combat drone wing in Borealis 2 orbit.  If any are sighted in the area, approach with \
			caution."
		if(3)
			msg = "Unidentified hackers have targeted a combat drone wing deployed around Borealis 2. If any are sighted in the area, approach with caution."
		if(4)
